...
- 時間足・指標の操作バーはPC（lg以上）では現在値の下、スマホでは時間足を最新足の日付の右側に配置する。モバイル（768px未満）の銘柄一覧・ロゴ検索は画面下部に常設するボトムナビから開く。ボタンは高さ48px以上とし、ホームインジケーターのセーフエリアを確保する。ヘッダーには同じ操作を重複表示しない。PCのサイドバーはヘッダーのボタンで開閉でき、閉じた分だけチャートを広げる。ロゴ検索はPCでは中央のダイアログ、モバイルでは下から開くシートとする
