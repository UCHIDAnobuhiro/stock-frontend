...
- スマホ（640px未満）の上部価格は前日比と同じ上昇・下落色で表示する。価格未取得時とPC版は通常のテキスト色とする
- スマホ（640px未満）はテクニカル指標のボタン・数値・線を表示しない。PCで有効にした設定は保持する。PCの指標値は四本値の下の固定高の行からポップオーバーで確認する。PCでは指標の追加・数値表示によってチャートの高さを変えない。指標値を開いたままチャートを操作でき、通常はカーソル移動で四本値と同じ足の値に追従し、クリック・タップで両方の数値を固定、再クリック・タップまたは「最新へ」で解除する。チャート外へ移動しても最後に選択した足を維持する
- 時間足・指標の操作バーはPC（lg以上）では現在値の下、スマホでは時間足を最新足の日付の右側に配置する。モバイル（768px未満）の銘柄一覧・ロゴ検索は画面下部に常設するボトムナビから開く。ボタンは高さ48px以上とし、ホームインジケーターのセーフエリアを確保する。ヘッダーには同じ操作を重複表示しない。PCのサイドバーはヘッダーのボタンで開閉でき、閉じた分だけチャートを広げる。ロゴ検索はPCでは中央のダイアログ、モバイルでは下から開くシートとする。モバイルのロゴ検索とウォッチリストはヘッダーの余白と閉じるボタンを揃え、上部バーの下方向ドラッグで閉じられる。企業分析の開始・完了時はシート内を分析欄の先頭へスクロールし、背景のページは動かさない
- スマホ（640px未満）のチャートの価格軸は初回表示の範囲を保持し、横スクロール・ピンチ操作・データ再取得・テーマ変更で自動拡縮しない。銘柄・時間足の切り替え時は新しいデータで初期化する。
- チャートは最新足・選択中・固定中を区別し、PCでは選択した足を確認できる。スマホ（640px未満）の始値・高値・安値・出来高は常に最新足を表示し、前後の矢印・「最新へ」は表示しない。銘柄・時間足の変更では選択をリセットし、指標・テーマの変更では表示範囲を維持する
...
- スマホ（640px未満）は終値の欄を出来高に置き換え、「始値・高値・安値・出来高」を表示する。出来高の専用行を設けずチャートの縦幅を確保する。日付の右側には時間足を表示する
