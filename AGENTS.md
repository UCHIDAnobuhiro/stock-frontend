...
- チャートは最新足・選択中・固定中を区別し、PCでは選択した足を確認できる。スマホ（640px未満）の四本値は常に最新足を表示し、前後の矢印・「最新へ」は表示しない。銘柄・時間足の変更では選択をリセットし、指標・テーマの変更では表示範囲を維持する
- ダークテーマの各色は `app/globals.css` の `.dark` トークンを正本とする
- スマホ（640px未満）の出来高は最新足の日付の右側に表示し、四本値の下には専用行を設けずチャートの縦幅を確保する。日付・出来高・時間足が収まらない幅では折り返す
