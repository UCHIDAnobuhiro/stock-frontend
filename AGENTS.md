...
- 時間足・指標の操作バーはPC（lg以上）では現在値の下、スマホでは時間足を最新足の日付の右側に配置する。スマホの銘柄・ロゴ検索はヘッダーから開き、ボトムナビは表示しない。PCのサイドバーはヘッダーのボタンで開閉でき、閉じた分だけチャートを広げる。ロゴ検索はPCでは中央のダイアログ、モバイルでは下から開くシートとする
