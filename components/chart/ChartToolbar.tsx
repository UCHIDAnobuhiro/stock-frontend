...
export function ChartToolbar({ isPending = false, readoutRef, isLoading: isChartLoading = false }: ChartToolbarProps) {
  ...
  return (
    <section className="grid shrink-0 grid-cols-1 gap-x-6 bg-[var(--color-surface-1)] px-4 pt-5 pb-4 xl:px-6 xl:pt-6 xl:contents" aria-label="銘柄情報と表示設定">
      ...
      {symbol && <div className="col-start-2 row-start-1 flex flex-col items-end gap-x-3 tabular-nums xl:mt-3 xl:flex-row xl:flex-wrap xl:items-baseline xl:gap-y-1">
        <span className={`whitespace-nowrap text-2xl font-semibold tracking-tight xl:text-4xl xl:text-[var(--color-text-primary)] ${quote ? quote.change >= 0 ? "text-[var(--color-bull)]" : "text-[var(--color-bear)]" : "text-[var(--color-text-primary)]"}`}>{quote ? quote.close.toLocaleString("ja-JP", { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : "—"}</span>
