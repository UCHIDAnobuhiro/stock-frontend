...
import LogoSearchContent from "@/components/logo/LogoSearchContent";
...
describe("LogoSearchContent", () => {
  ...
  it("検索前は未検出メッセージを表示しない", () => {
    render(<LogoSearchContent open onOpenChange={vi.fn()} />);
  ...
  it("分析開始・完了時に分析欄へ移動し、通常の再描画では移動しない", async () => {
    ...
    try {
      const { rerender } = render(<LogoSearchContent open onOpenChange={vi.fn()} />);
      ...
      rerender(<LogoSearchContent open onOpenChange={vi.fn()} />);
      await waitFor(() => expect(scrollTo).toHaveBeenCalledOnce());
      rerender(<LogoSearchContent open onOpenChange={vi.fn()} />);
  ...
  it("分析tickerと完全一致する銘柄でチャートを開く", () => {
    ...
    render(<LogoSearchContent open onOpenChange={onOpenChange} />);
